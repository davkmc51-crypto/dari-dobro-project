...
export const Fundraisers = ({ onDonate }: FundraisersProps) => (
  <section id="fundraisers" className="border-t bg-muted/60 px-5 py-20 lg:px-14 lg:py-28">
    <div className="mx-auto max-w-[1400px]">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <SectionHeading
          ...
          title={
            <>
              Что мы собираем <em className="font-medium italic text-accent">прямо сейчас</em>
            </>
          }
        />
        <button
        ...
        {FUNDRAISERS.map((f, i) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="mt-4 font-display text-2xl font-bold leading-tight text-foreground">{f.title}</h3>
              <p className="mt-2 flex-1 text-[0.95rem] text-muted-foreground">{f.desc}</p>

              <div className="mt-6">
                <div className="flex items-baseline justify-between">
                  <span className="font-display text-2xl font-bold text-foreground">{fmt(f.raised)} ₽</span>
                  <span className="text-sm text-muted-foreground">из {fmt(f.goal)} ₽</span>
                </div>
                <div
                ...
                <div className="mt-2 text-[0.78rem] text-muted-foreground">Собрано {percent}%</div>
              </div>
